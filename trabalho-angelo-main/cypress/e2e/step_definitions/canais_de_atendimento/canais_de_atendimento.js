...
When("Click on canais de atendimento", () => {
  ...
  cy.get(
    ".rodape-links > :nth-child(2) > :nth-child(1) > ul > :nth-child(1) > a"
  ).click();
});
...
When("Fill in the mandatory fields", () => {
  cy.get("#authorName").type("Bia D'Emery");
  cy.get("#authorEmail").type("bia@gmail.com");
  cy.get("#category").select("GEOGRAPHY");
  cy.get("#title").type("Projeto de QA");
  cy.get("#message").type("Este é um projeto da disciplina de QA");
